...
void test_max_size(void (*check)(test_type &, ref_type &)) {
  test_type test(10, 2);
  ...
  unittest::assert(test.max_size() == test_ref.max_size(),
                   "wrong max size value");
}

template <typename test_type, typename ref_type>
void test_size(void (*check)(test_type &, ref_type &)) {
  test_type test;
  ref_type test_ref;
...
template <typename test_type, typename ref_type>
void test_capacity(void (*check)(test_type &, ref_type &)) {
  test_type test;
  ref_type test_ref;
  check(test, test_ref);
...
template <typename test_type, typename ref_type>
void test_reserve(void (*check)(test_type &, ref_type &)) {
  test_type test;
  ref_type test_ref;
  check(test, test_ref);
...
template <typename test_type, typename ref_type>
void test_empty(void (*check)(test_type &, ref_type &)) {
  test_type test;
  ref_type test_ref;
  check(test, test_ref);
